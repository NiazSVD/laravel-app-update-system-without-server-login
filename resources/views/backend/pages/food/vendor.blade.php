...
@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center border-bottom">
            <h3 class="card-title">খাবার তালিকা</h3>
            <div class="d-flex justify-content-end mb-3">
                <button type="button" class="btn btn-primary me-2" data-bs-toggle="modal" data-bs-target="#addTodayFoodModal">
                    <i class="bi bi-calendar-plus me-1"></i> আজকের খাবার
                </button>
                    ...
                    <tbody>
                        @forelse($get_foods as $get_food)
                            <tr>
                                ...
                                <td>
                                    @php
                                        $isToday = $get_food->meal_date && \Carbon\Carbon::parse($get_food->meal_date)->format('Y-m-d') == \Carbon\Carbon::today()->format('Y-m-d');
                                    @endphp

                                    <span class="badge {{ $isToday ? 'bg-success' : 'bg-danger' }}">
                                        {{ $isToday ? 'হ্যাঁ' : 'না' }}
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <button type="button" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                        ...
                                        data-food-name="{{ $get_food->name }}" data-food-price="{{ $get_food->price }}"
            ...
            <div class="modal-content">
                <form action="{{ route('admin.food.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-header">
                        <h5 class="modal-title">নতুন খাবার যোগ করুন</h5>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                <label class="form-label">মজুত</label>
                                <input type="number" step="0.01" class="form-control" name="stock" required>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">খাবারের বিবরণ</label>
                            <textarea class="form-control" name="description" rows="3" required></textarea>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">ছবি<small class="text-muted">(Recommended: JPG, PNG,
                                    JPEG)</small></label>
                ...
                <form id="editFoodForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3 mb-3">
                                <label class="form-label">মজুত</label>
                                <input type="number" step="0.01" class="form-control" name="stock" id="edit_food_stock" required>
                            </div>
                        ...
                        <div class="mb-3">
                            <label class="form-label">নতুন ছবি <small class="text-muted">(Recommended: JPG, PNG,
                                    JPEG)</small></label>
                            <input type="file" class="form-control" name="image" accept="image/*">
                            <small class="text-muted"></small>
                        </div>
                        ...
                        <div class="mb-3">
                            <label class="form-label">পুরাতন ছবি</label>
                            <div id="current_image_container" class="mt-2">
                                ...
                                <div id="no_image_message" class="text-muted">কোন ছবি নেই </div>
                            </div>
                        </div>
    ...
    {{-- Add today Food Modal --}}
    <div class="modal fade" id="addTodayFoodModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{ route('vendor.today.food.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="category_id" id="categoryIdInput">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            আজকের খাবার — {{ now()->locale('bn')->translatedFormat('d F Y (l)') }}
                        </h5>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="categorySelect" name="category_id" required>
                            <option value="">-- ক্যাটেগরি নির্ধারণ করুন --</option>
                            @foreach ($get_categories as $category)
                                <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                        </select>
                    ...
                    {{-- Foods --}}
                    <div class="mb-3">
                        <label class="form-label">খাবার সমূহ:</label>
                        <div id="foodsList">
                            <p class="text-muted">আগে ক্যাটেগরি নির্বাচন করুন</p>
                        </div>
                    </div>
...
@section('script')
    <script>
        ...
        ///category wish food show
        $('#categorySelect').on('change', function () {

            let categoryId = $(this).val();
            let foodList = $('#foodsList');
            $('#categoryIdInput').val(categoryId);

            foodList.html('<p>Loading...</p>');

            if (!categoryId) {
                foodList.html('<p class="text-muted">আগে ক্যাটেগরি নির্বাচন করুন</p>');
                return;
            }

            $.ajax({
                url: `/vendor/category-wise-food/${categoryId}`,
                type: 'GET',
                success: function(foods) {

                    if (foods.length === 0) {
                        foodList.html('<p class="text-danger">এই ক্যাটেগরি তে কোন খাবার নাই</p>');
                        return;
                    }
                    ...
                    foods.forEach(food => {
                        let checked = food.checked ? 'checked' : '';
                        html += `
                            <div class="form-check">
